...
public class CustomHorizontalProgresWithNum extends ProgressBar {
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        canvas.drawRoundRect(mRectF, 20, 20, mPaint);//圆角矩形
        //绘制进度
        mPaint.setColor(HorizontalProgresTextColor);
        ...
        if (progressX + HorizontalProgresTextOffset + textWidth < getWidth() - getPaddingLeft() - getPaddingRight()) {//进度走完了,不再画未走完的
            ...
            RectF mRectF2 = new RectF(progressX + HorizontalProgresTextOffset + textWidth,
                    getPaddingTop() - HorizontalProgresUnReachHeight / 2, realWidth,
                    HorizontalProgresUnReachHeight / 2);//圆角 int left, int top, int right, int bottom
